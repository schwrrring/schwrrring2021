...
void ClipComponent::mouseDown(const MouseEvent &event) {
    ...
    if (event.mods.isRightButtonDown()) {  // or possibly isRightButtonDown()

        const std::unique_ptr<AlertWindow> w(LookAndFeel::getDefaultLookAndFeel()
                                                     .createAlertWindow(TRANS("Chord Naming"),
                                                                        TRANS("Name your chord and thank Erik that you can do so :)"),
                                                                        {}, {}, {}, AlertWindow::QuestionIcon, 0,
                                                                        nullptr));

        w->addTextEditor("setName", "name", TRANS("Name:"));
...
//        auto *aw = new AlertWindow(TRANS("New TEst"),
...
//    auto uiBehaviour = new tracktion_engine::UIBehaviour();
//    uiBehaviour->showWarningAlert("test", "fill out the form");
        ...
        if (af.getFile().existsAsFile() || (!wac->usesSourceFile())) {
            if (af.isValid()) {
                const te::AudioFile proxy(
                        (wac->hasAnyTakes() && wac->isShowingTakes()) ? wac->getAudioFile() : wac->getPlaybackFile());
...
//    todo: farbe setzbar machen
    if (track->state.hasProperty(tracktion_engine::IDs::colour)) {
        g.fillAll(Colour::fromString(track->state.getProperty(tracktion_engine::IDs::colour).toString()));
    } else {
